...
public class TwoDBlockMatrix{
	...
	public static TwoDBlockMatrix buildTwoDBlockMatrix(java.io.InputStream in) throws IOException{
		// float [][]matrix;
		if(in==null){
	...
	public TwoDBlockMatrix transpose(){
		...
		float [][]matrixT = new float[this.matrix[0].length][this.matrix.length];
			...
			for(int i = 0; i < this.matrix.length; i++){
				for(int j = 0; j < other.matrix[0].length; j++){
					...
					for(int t = 0; t < other.matrix.length; t++){
						finals[i][j] = finals[i][j] + this.matrix[i][t]*other.matrix[t][j];
					}	
				}
			}
		}	
